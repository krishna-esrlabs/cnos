...
class CustomDINOv3(pl.LightningModule):
    def __init__(
        self,
        model_name,
        model,
        ...
        logging.info(f"Init CustomDINOv3 done!")
        ...
        logging.info(
            f"Init CustomDINOv3 with full size={descriptor_width_size} and proposal size={self.proposal_size} done!"
        )

    def process_rgb_proposals(self, image_np, masks, boxes):
        """
        1. Normalize image with DINOv3 transfom
